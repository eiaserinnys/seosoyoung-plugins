...
async def execute_interventions(
    ...
    """InterventionAction 리스트를 슬랙 API로 발송합니다. (plugin_sdk 사용)
...
async def send_debug_log(
    ...
    """디버그 채널에 관찰 결과 로그를 전송합니다 (Block Kit 형식, plugin_sdk 사용)."""
...
async def send_collect_debug_log(
    ...
    """메시지 수집 시 디버그 채널에 로그를 전송합니다 (Block Kit 형식, plugin_sdk 사용)."""
    if not debug_channel:
        return
...
async def send_digest_skip_debug_log(
    ...
    """소화 스킵(임계치 미달) 시 디버그 채널에 로그를 전송합니다 (Block Kit 형식, plugin_sdk 사용)."""
    if not debug_channel:
        return
...
async def send_intervention_probability_debug_log(
    ...
    """확률 기반 개입 판단 결과를 디버그 채널에 기록합니다 (Block Kit 형식, plugin_sdk 사용)."""
    if not debug_channel:
        return
...
async def send_multi_judge_debug_log(
    ...
    """복수 판단 결과를 메시지별 독립 블록으로 디버그 채널에 전송합니다 (plugin_sdk 사용)."""
    if not debug_channel:
        return
